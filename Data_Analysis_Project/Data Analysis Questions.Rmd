...
ggplot(joined_death_rate) +
  ...
  labs(x="Poverty Rate", y="Opiioid Death Rate", title="County population and total opioids in 2012", caption = "Source: DEA ARCOS database, via Washington Post", fill="buyer_county") +
...
ggplot(joined_death_rate_to_pills_per_person) +
  ...
  labs(x="Pills per person", y="Opiioid Death Rate", title="County population and total opioids in 2012", caption = "Source: DEA ARCOS database, via Washington Post", fill="buyer_county") +
  scale_y_continuous(labels = comma) +
  scale_x_continuous(labels = comma) + 
  theme(axis.text.x = element_text(angle = 45, hjust = 1)) +
  geom_smooth(aes(pills_per_person, `Crude Rate`), method = "lm", se = FALSE)
joined_death_rate_to_pills_per_person %>% 
  select(pills_per_person, `Crude Rate`) %>%
  correlate() 
```
