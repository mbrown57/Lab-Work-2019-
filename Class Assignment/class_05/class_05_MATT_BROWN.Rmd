...
```{r}
buyer_addresses <- read.csv("data/buyer_addresses.csv")
buyer_totals <- read.csv("data/buyer_totals.csv")
buyer_annual_by_year <- read.csv("data/buyer_annual_by_year.csv")
state_population_per_year <- read.csv("data/state_population_per_year.csv")
```
## Examine the Data
...
```{r}
#The buyer annual by year is probably higher than buyer total because it is counting each individual dosage unit per the buyer rather than total pills. The buyer address will be higher because it tracks each address of the buyer annually by year and not just the totals 
```
...
#Retail Pharmacy (VA consolidated mail outpatient pharmacy) Carleston SC, VA CMOP, North Charleston. This was one of the higher percentages of pills shipped to this area
```
...
```{r}
   #Mathur, Rakesh K MD had 337450 pills shipped to him. The next highest doctor had 212800 pills. He is on a probation. The Board determined that the physician’s conduct constituted a substantial likelihood of a risk of serious harm to the public health, safety and welfare based on the physician’s prescribing of opiod medications while failing to use reasonable safeguards or to exercise due diligence.I would verify by address of pharmacy and middle/first name. 
...
#an inner join will only match records between both tables
buyer_totals %>%
  left_join(buyer_addresses, by="buyer_dea_no")
#a left join will match the buyer totals with buyer addresses that match all of the buyer total table items
buyer_totals %>%
  left_join(buyer_addresses, by="buyer_dea_no") %>%
  filter(is.na(buyer_address1))
#this join means that all totals have addresses where they match and the 0 records returned just means there are no null addresses
buyer_totals %>%
  anti_join(buyer_addresses, by="buyer_dea_no")
#an antijoin will check if there are any buyer addresses that do not match the records 
```
...
#inner join will return all the matching records in both records 
buyer_addresses %>%
  left_join(buyer_totals, by="buyer_dea_no")
#left join will take all of the buyer addresses and return all of the matching buyer totals to the addresses
buyer_addresses %>%
  left_join(buyer_totals, by="buyer_dea_no") %>%
  filter(is.na(total_pills))
#A left join will with the filter will use all the addresses and join all matching buyer totals while showing the buyers that do not have transactions in the totals table, and there are many
buyer_addresses %>%
  anti_join(buyer_totals, by="buyer_dea_no")
#an anti join will do basically the same thing and show any buyers that don't have any pills being bought in the total table, and there are a lot again
