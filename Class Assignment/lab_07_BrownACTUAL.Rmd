...
ggplot(aes(fill = pills_per_person)) + 
    geom_sf(lwd = 0) + 
  theme_map() + 
labs(fill='Pills per Person in 2009',title="The highest rates of opioid deaths were in Appalachian region", subtitle = "WV and Kentucky had the highest opioid death rates per person in 2009", caption = "Source; arcos database") +
...
geo_pills_population_state %>%
  ...
  geom_sf(lwd = 0) +
  theme_map() +
  labs(fill='Pills per Person',title="Pills per person from 2006-2012 increased progressively especially in the appalachian region", subtitle = "Kentucky and 
...
geo_pills_population_county_maryland %>%
  ...
  geom_sf(lwd = 0) + 
  theme_map() + 
  labs(fill='pills per person in each county',title="The trend jumped between 2006-2012 and started at a slower rate ", subtitle = "Amount of pills shipped to Maryland counties 2006-2012", caption = "Source: ARCOS database") +
  theme(legend.position="bottom") +
  scale_fill_viridis_c(option = "magma",labels = comma)
```
